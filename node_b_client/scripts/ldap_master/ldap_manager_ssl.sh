...
# Versão:      6.1 (Fix: AppArmor Standard Path)
# Descrição:   Gerenciador OpenLDAP Master SSL.
#              - Executa como ROOT (para evitar erros de permissão).
#              - Usa caminhos PADRÃO (/var/lib/ldap) para satisfazer AppArmor.
# -------------------------------------------------------------------------
...
rootdn      "$ROOT_DN"
rootpw      "$ROOT_PW"
directory   "$DATA_DIR"   # Agora aponta para /var/lib/ldap
...
start_master() {
    ...
    # Roda como root (sem -u/-g), mas usando a pasta padrão
...
clear
echo "========================================================"
echo "   GERENCIADOR MASTER SSL v6.1 (Standard Path)"
echo "========================================================"
